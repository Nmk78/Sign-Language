...
if (!isset($db_error)) {
    $stmt = $conn->prepare("
        SELECT 
            CONCAT(
                LPAD(FLOOR(HOUR(cl.completed_at) / 3) * 3, 2, '0'), 
                ':00 - ', 
                LPAD(FLOOR(HOUR(cl.completed_at) / 3) * 3 + 2, 2, '0'), 
                ':59'
            ) AS time_range,
            COUNT(cl.id) AS lesson_count
        FROM completed_lessons cl
        JOIN lesson l ON cl.lesson_id = l.id
        WHERE cl.completed_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)
        GROUP BY time_range
...
// Example output format for chart
$chart_data = [];
